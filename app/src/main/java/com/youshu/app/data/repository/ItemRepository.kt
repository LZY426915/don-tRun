...
class ItemRepository @Inject constructor(
    ...
    suspend fun markAsUsed(id: Long, rating: Int? = null, reviewNote: String? = null) {
        val ratedAt = rating?.let { System.currentTimeMillis() }
        itemDao.updateStatusAndRating(id, Item.STATUS_USED_UP, rating, ratedAt, reviewNote.orEmpty())
    }

    suspend fun rateUsedItem(id: Long, rating: Int, reviewNote: String? = null) =
        itemDao.updateRating(id, rating, System.currentTimeMillis(), reviewNote.orEmpty())
    ...
    fun getItemsByLocationName(locationName: String): Flow<List<ItemDetail>> =
        itemDao.getItemsByLocationName(locationName)

    fun getItemsByCategoryName(categoryName: String): Flow<List<ItemDetail>> =
        itemDao.getItemsByCategoryName(categoryName)
}
